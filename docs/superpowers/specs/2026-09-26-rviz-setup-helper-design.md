...
- **예비 경로 [직접 설치]:** Ubuntu 공식 22.04 WSL 루트 파일(약 230MB, SHA256 확인)을 받아 전용 배포판 `MirobotSketch-ROS`로 가져온 뒤, 새 콘솔에서 그 안의 root로 `packaging/wsl/setup_ros_env.sh --image`를 실행합니다. 이미 있으면 스크립트만 다시 실행합니다(끝난 단계는 건너뜀). 사용자의 기존 배포판은 건드리지 않고, 비밀번호·추가 관리자 승인이 없습니다. (최종 검토 후 변경: 처음 설계는 사용자의 `Ubuntu-22.04`에 설치했는데, 기존 배포판을 건드리지 않는다는 원칙과 충돌하고 WSL이 없을 때 관리자 승인이 한 번 더 필요했음)
