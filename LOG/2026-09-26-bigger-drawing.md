...
- **넓은 범위를 지붕 모양 영역으로 (`limits_pending_verification`):**
  - 좌우 ±125mm, 아래 −85mm, 위는 가로 위치별 한계입니다(가운데 +57.5 → |x| 60에서 +55 → 90에서 +52.5 → 끝 +42.5, 점 사이 직선).
  - 한계는 도달 지도(2.5mm 격자)에서 관절 여유가 7° 이상인 높이에서 2.5mm를 뺀 값이고, 종이 안전 여백 20mm 안으로 잘랐습니다.
  - 지붕은 **볼록**하게(기울기가 점점 가파르게) 잡았습니다. 실행기는 선의 꼭짓점만 검사하므로, 볼록해야 꼭짓점 두 개가 안에 있을 때 그 사이 선분도 안에 있습니다(최종 검토에서 찾은 문제, 테스트로 고정).
...
- **자동 구도와 얼굴 세밀:** 종이 위 얼굴 크기와 펜 굵기 환산에 요청한 크기가 아니라 영역에 맞춘 실제 크기를 씁니다(세로 그림은 요청보다 작게 그려짐).
- **에이전트:** 설정 범위 설명에 그리기 크기 최대 250mm가 반영됩니다.
- **화면:**
...
- **새 테두리 경로 시뮬레이션:** PASS. 가장 빠듯한 B(J5)가 한계까지 8.9° 남습니다.
- **실제 그림 (긴 변 250mm 요청, 넓은 범위):**

| 그림 | 실제 크기 | 배치 | 예상 시간 | 시뮬레이션 |
|---|---|---|---|---|
| illust1 (100mm, 이전 기본) | 95×100mm | 가운데 | 23.5분 | PASS (B 12.0°) |
| illust1 | 132×139.5mm | 15mm 아래 | 27.7분 | PASS (B 10.2°) |
| photo1 (가로) | 224×132mm | 19mm 아래 | 28.0분 | PASS (B 12.0°) |
| photo2 (세로, 자동 구도 상반신) | 116×140mm | 15mm 아래 | 47.4분 | PASS (B 10.0°) |
...
## 남은 작은 문제 (최종 검토, 이번에는 고치지 않음)
- 자동 구도는 얼굴 크기를 원본 이미지 비율로 추정합니다. 배경 제거 뒤 획 상자가 세로로 좁아지면 실제 얼굴이 더 작아 "전체"를 고를 수 있습니다.
- 사용자가 예전 절차대로 ±60을 실물 확인하고 `limits`를 60으로 올린 경우에도, 넓은 범위가 (60, 60)이면 새 지붕 영역으로 바뀝니다. 가운데 위쪽은 57.5로 더 좁아집니다(안전 쪽).
- 명령줄 `mirobot-strokes --box 250 250`은 영역 맞추기를 하지 않아 가운데에 둡니다. 큰 그림은 GUI나 에이전트로 만드세요.
- 설정 파일의 지붕 값을 손으로 잘못 고치면(빈 목록 등) 검증 없이 오류가 납니다.
- `state()["pending_limit_mm"]`(250)는 폭만 뜻합니다. 모양은 `pending_limit_outline_mm`를 보세요.
